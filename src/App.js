...
// Liste initiale des films
    { id: 1, title: 'Blue Beetle', description: "Fraîchement diplômé de l’université, Jaime Reyes rentre chez lui, plein d’ambitions, mais il découvre que la situation a bien changé depuis son départ. Tandis qu’il cherche sa place dans le monde, le destin s’en mêle : Jaime se retrouve par hasard en possession du Scarabée, une ancienne relique d’une biotechnologie extraterrestre. Dès lors que le Scarabée choisit de faire de Jaime son hôte, le jeune homme se voit revêtu d’une armure hors du commun qui lui octroie des pouvoirs extraordinaires – et imprévisibles. Tout bascule alors pour Jaime qui devient le super-héros Blue Beetle", posterURL: 'https://fr.web.img3.acsta.net/c_310_420/pictures/23/08/11/15/51/5392555.jpg', rate: '3' },
    { id: 2, title: 'Barbie', description: "A Barbie Land, vous êtes un être parfait dans un monde parfait. Sauf si vous êtes en crise existentielle, ou si vous êtes Ken.", posterURL: 'https://fr.web.img2.acsta.net/c_310_420/pictures/23/06/16/12/04/4590179.jpg', rate: '4' },
...
useEffect(() => {
  const updatedMovies = movies.filter(movie => {
    ...
    const rateMatch = movie.rate === rate || rate === '';
...
return (
  <div className="App bg-light mb-5">
    <Navbar variant="light" className="bg-danger bg-gradient d-flex justify-content-between px-5 sticky-top">
      <Navbar.Brand className="adlam text-light">Go My Flix</Navbar.Brand>
      <Filter
        onFilter={({ title, rate }) => {
          setTitle(title);
          setRate(rate);
